...
function ImportRepository () {
	...
	const onFormChanged = (values: any) => {
		setImportType(values.importType);
	};

	const [{ data : repository, loading : loadingRepository }, fetch] = useGithubApiRequest(`/repos/${repositoryFullName}`);
	const [{ data : resultImportApplication, loading : loadingImportApplication }, importApplication] = useLoggedApiRequest({
		url: `/applications/import`,
		method: 'POST'
	}, { manual: true});
	const [{ data : resultImportMicrofrontend, loading : loadingImportMicrofrontend }, importMicrofrontend] = useLoggedApiRequest({
	...
	if (!loadingImportApplication && resultImportApplication) return <Redirect to={`../../../application/${resultImportApplication.id}`} />;
	if (!loadingImportMicrofrontend && resultImportMicrofrontend) return <Redirect to={`../../../microfrontend/${resultImportMicrofrontend.id}`} />;
  ...
  return (
	<Page title="Import repository">
		...
		>
			...
			<div>
				<Button htmlType="submit" type="primary" disabled={loadingImportMicrofrontend || loadingImportApplication}>Import</Button>
			</div>
		</Form>
	</Page>
  );
