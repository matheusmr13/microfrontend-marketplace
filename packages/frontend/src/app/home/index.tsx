...
function FullApp() {
  ...
  const selectedMenuIndex = reverseItems.length - reverseItems.findIndex(item  => history.location.pathname.replace(match.path, '').startsWith(item.url)) - 1;

  return (
    <>
      <Menu
        style={{ minWidth: 256 }}
        selectedKeys={[selectedMenuIndex.toString()]}
        mode={'vertical'}
        theme={'dark'}
      >
        { items.map((item, i) => (
